...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, Q, K, V, attn_mask):
        '''
        Q : [batch_size, n_heads, len_q, d_q(=d_k)]
...
class MultiHeadAttention(nn.Module):
    def __init__(self):
        ...
        # this ensure d_v can be set by whatever dimension we want
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, dec_inputs, enc_outputs, dec_self_attn_mask, dec_enc_attn_mask):
        ...
        enc_outputs : [batch_size, sec_len, d_model]
        dec_self_attn_mask : [batch_size, tgt_len, tgt_len]
        dec_enc_attn_mask : [batch_size, tgt_len, src_len]
        '''
        # print(dec_self_attn_mask, "asdas", dec_enc_attn_mask)
...
class Decoder(nn.Module):
    ...
    def forward(self, dec_inputs, enc_inputs, enc_outputs):
        ...
        # print(dec_self_attn_mask, dec_self_attn_subsequence_mask)
...
def Test_decoder(model, enc_input, start_symbol):
    """
...
greedy_dec_input = Test_decoder(model, enc_inputs[0].view(1, -1).cuda(), start_symbol=tgt_vocab["S"])
